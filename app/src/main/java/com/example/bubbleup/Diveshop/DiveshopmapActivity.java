...
public class DiveshopmapActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_diveshopmap);

        diveshop = (Diveshop) this.getIntent().getSerializableExtra("diveshop");


        tvDstitle = findViewById(R.id.tvDstitle);

        SupportMapFragment mapFragment = (SupportMapFragment)getSupportFragmentManager().findFragmentById(R.id.fmMap2);

        tvDstitle.setText(diveshop.getDs_name());

        mapFragment.getMapAsync(this);



        String strLatLng = diveshop.getDs_latlng().substring(1,diveshop.getDs_latlng().length()-1);
    ...
    private void setupMap() {
        ...
        map.setInfoWindowAdapter(new DiveshopmapActivity.MyInfoWindowAdapter());

        DiveshopmapActivity.MyMarkerListener listener = new DiveshopmapActivity.MyMarkerListener();
    ...
    private void addMarkersToMap() {
        marker = map.addMarker(new MarkerOptions()
                ...
                .position(latLng)
                // 設定標記標題
                .title(diveshop.getDs_name())
                // 設定標記描述
                .snippet(diveshop.getDsinfo())
                // 設定標記圖示
    ...
    private class MyInfoWindowAdapter implements GoogleMap.InfoWindowAdapter {
        ...
        MyInfoWindowAdapter() {
            infoWindow = LayoutInflater.from(DiveshopmapActivity.this)
...
//            ivLogo.setImageResource(logoId);
    ...
    private class MyMarkerListener implements GoogleMap.OnMarkerClickListener,
        ...
        public boolean onMarkerClick(Marker marker) {
            Toast.makeText(DiveshopmapActivity.this, marker.getTitle(), Toast.LENGTH_SHORT).show();
            return false;
        }
